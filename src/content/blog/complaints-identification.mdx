...
**Why TF-IDF over transformer embeddings?** A reasonable question — models like BERT produce dense vector representations that capture semantic relationships TF-IDF can't. "I'm fed up with this" and "this is completely unacceptable" would land close together in BERT space but look like entirely unrelated features in a TF-IDF matrix. The synonym normalization step above is essentially us doing manually what BERT does natively. But there are two practical reasons we chose TF-IDF for this use case. First, **explainability**: TF-IDF features are individual words. When the model flags a call, you can trace the prediction back to specific terms — "escalate," "supervisor," "frustrated" — that a compliance officer can verify against the transcript. BERT embeddings are 768-dimensional vectors where no single dimension maps to a human-interpretable concept. The moment you feed those into a Random Forest, your "explainable" model is making decisions based on a representation that's as opaque as an LLM's internals — you've accepted transformer-level opacity without getting the full comprehension benefits of actually using the LLM. Second, **document length**: BERT has a hard token limit (512 tokens, roughly 300-400 words). A call transcript can run thousands of words, forcing you to chunk the transcript into windows, embed each separately, and average the vectors — which dilutes complaint signal if the customer's frustration lives in two minutes of an otherwise pleasant 20-minute call. TF-IDF has no such constraint; it produces a single sparse vector across the entire document regardless of length. It's a simpler representation, but for a regulatory control where you need to explain *why* a call was flagged and you're processing documents of highly variable length, the practical tradeoffs favor it.
